...
const ALLOWED_FIELDS = ["name","age","gender","city","chronic_conditions","allergies","emergency_contact"];
...
module.exports = async function symptomAgent(text, state = {}) {
  ...
  if (state.completed && state.personalFormGenerated) {
    return { type: "done", reply: "🎉 Your profile is complete!", options: ["Start new chat", "Book Appointment"], update: state };
  }
  ...
  if (!state.asked.includes("main_symptom")) {
    ...
    return {
      ...
      options: ["Fever","Cough","Cold","Headache","Stomach pain","Body pain","Breathing issue","Other"],
      update: state
    };
  }

  // Detect & store symptom
  ...
  if (main === "fever") {
    ...
    if (!state.asked.includes("fever_duration")) {
      ...
      return { type:"message", reply:"How long have you had fever?", options:["< 24 hours","1–3 days","> 3 days","Not sure"], update:state };
    }
    if (!fever.duration && t) fever.duration = text;

    if (!state.asked.includes("fever_temp")) {
      askOnce("fever_temp");
      return { type:"message", reply:"Highest recorded temperature?", options:["< 99°F","99–100.9°F","101–102°F","> 102°F","Not measured"], update:state };
    }
    if (!fever.temperature && t) fever.temperature = text;

    if (!state.asked.includes("fever_meds")) {
      askOnce("fever_meds");
      return { type:"message", reply:"Did you take medicine?", options:["Yes, helped","Yes, no improvement","No"], update:state };
    }
  ...
  // Ask other symptoms once
  if (!state.asked.includes("other_symptoms")) {
    askOnce("other_symptoms");
    return { type:"message", reply:"Any other symptoms?", options:["Cold","Cough","Headache","Body pain","Breathing issue","No"], update:state };
  }
  if (!state.answers.otherSymptoms && t) state.answers.otherSymptoms = t.startsWith("no") ? "none" : text;

  // Generate personal form
  if (!state.personalFormGenerated) {
    ...
    return { type:"form", reply:"Please fill your personal details 👇", form, update:state };
  }

  return { type:"message", reply:"Please fill your personal details to continue.", update:state};
};
...
async function generatePersonalForm(state) {
  ...
  try {
    const system = `
Choose personal fields from whitelist ONLY:
${ALLOWED_FIELDS.join(", ")}
Return JSON: {"fields":[{"name":"name","required":true}]}
No comments.
`.trim();
...
const label = (n)=>({name:"Full Name",age:"Age",gender:"Gender",city:"City",chronic_conditions:"Chronic Conditions",allergies:"Allergies",emergency_contact:"Emergency Contact"}[n]||n);
